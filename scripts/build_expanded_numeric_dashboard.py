...
def sanitize(report, *, allow_incomplete=False):
    ...
    result = {"schema_version": 1, "scope": "Exploratory numerical classification and cached-label Jev review",
    ...
    for row in report["runs"]:
        ...
        require(row["status"] in {"complete", "running", "stopped_after_three_consecutive_errors", "pending: no complete audited artifact"}, "Unexpected run status")
        if complete:
