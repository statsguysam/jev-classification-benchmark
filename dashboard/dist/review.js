...
const datasets = {breast_cancer:'Breast Cancer · binary',wine:'Wine · multiclass',sst2:'SST-2 · binary',trec:'TREC · multiclass',titanic:'Titanic · historical mixed tabular'};
...
function renderControls(){
 ...
 el('control-arms').innerHTML=table(['Jev input','Saved / planned','Status','Selected metric','95% interval','Failures'],runs.map(r=>{
  const done=r.status==='complete', m=done?r.metrics:null, ci=done?r.group_bootstrap?.metrics?.[metric]?.ci95:null;
  return [armNames[r.arm]??r.arm,r.saved_requests==null?'Not reported':`${r.saved_requests}/${r.expected_requests}`,
   done?'Complete':'Pending',done?fmt(m?.[metric],metric):'Pending',done?interval(ci,metric):'Pending',done?count(m?.n_failures):'Pending'];
 }));
 const comparisons=c.comparisons?.filter(r=>r.dataset===dataset)??['no_proposal','shuffled'].map(b=>({a:'actual',b,status:'pending'}));
 el('control-contrasts').innerHTML=table(['Paired contrast: A − B','Status','Difference','95% paired interval','Corrected / harmed'],comparisons.map(r=>{
  const done=r.status==='complete', estimate=metric==='balanced_accuracy'?r.balanced_accuracy_delta:r.paired_group_bootstrap?.metrics?.[metric]?.estimate;
  const ci=r.paired_group_bootstrap?.metrics?.[metric]?.ci95, t=r.transitions_B_to_A;
...
function renderControlRecovery(){
 ...
 if(!r||r.status!=='complete'){
  ...
  el('control-recovery-arms').innerHTML='';el('control-recovery-contrasts').innerHTML='';return;
 }
...
function renderRecovery(){
 ...
 if(!r||r.status==='not_available'){
  ...
  el('recovery-table').innerHTML='';return;
 }
 ...
 el('recovery-table').innerHTML=table(['Condition · full rows','Status','Original snapshot','First actual call','Recovered','Recovered − first · 95% CI','Failures: original / first / recovered','Corrected / harmed'],r.conditions.map(c=>{
  ...
  return [`${datasets[c.dataset]??c.dataset} / ${model} / ${c.shots_per_class??'N/A'} examples per class / n=${c.n_rows??'N/A'}`,
   ...
   done?`${count(c.corrected)} / ${count(c.harmed)}`:'Pending'];
 }));
...
async function init(){
 ...
 el('condition').addEventListener('change',renderComparison); el('gate').addEventListener('change',renderGate); el('control-dataset').addEventListener('change',renderControls);
 el('metric').addEventListener('change',()=>{renderComparison();renderGate();renderControls();renderRecovery();});renderComparison();renderGate();renderControls();renderRecovery();
