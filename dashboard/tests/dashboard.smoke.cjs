...
const document={getElementById:id=>{if(!elements.has(id))elements.set(id,{textContent:'',hidden:true,disabled:false,innerHTML:'',clientWidth:850});return elements.get(id);}};
const captured=[];
const sandbox={document,console,setTimeout:()=>0,clearTimeout:()=>{},capture:(...args)=>captured.push(args),fixture:data};
