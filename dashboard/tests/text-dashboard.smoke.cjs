...
const source = fs.readFileSync(path.join(dist, 'text.js'), 'utf8');
...
async function harness(data, query = '') {
  ...
  const context = vm.createContext({
    ...
    async fetch(url) { assert.equal(url, 'text-data.json'); capture.fetches++; return {ok: true, async json() { return data; }}; }
  });
...
async function verifyView(h, data, state) {
  ...
  let referenceMarker = 0, reviewMarker = 0;
...
async function nullChecks(fixture) {
  const baseState = {dataset:'sst2',shots:'4',model:sourceModels[0].id,metric:'accuracy',view:'pairs',nativeBudget:'4'};
...
function completeFixture() {
  const data={schema_version:1,completion:{status:'complete',complete_runs:68,expected_runs:68,complete_comparisons:72,expected_comparisons:72},
    ...
    datasets:[{id:'sst2',label:'Synthetic binary fixture',n_test:200,n_classes:2,n_features:null,feature_description:'Text · TF-IDF for classical models',full_training_labels:10000},
      {id:'trec',label:'Synthetic multiclass fixture',n_test:200,n_classes:6,n_features:null,feature_description:'Text · TF-IDF for classical models',full_training_labels:4886}],
...
function partialFixture(complete, sourceReady=false) {
  ...
  data.completion.status='in_progress_or_incomplete';data.completion.complete_runs=sourceReady?44:36;
  data.completion.complete_comparisons=data.comparisons.filter(c=>c.status==='complete').length;
  data.costs.status='blocked_no_credit';data.costs.new_review_calls=0;
  data.costs.known_reported_api_usd='0';data.costs.unknown_cost_calls=0;
  return data;
}

function knownMeasuredReferences(data) {
  const nativeCounts={sst2:[[99,158],[106,152],[89,138],[95,135]],trec:[[100,173],[98,162],[70,141],[75,145]]};
  const nativeModels=['logistic_regression','random_forest','xgboost','lightgbm'];
  for(const ds of data.datasets) {
    assert.equal(ds.n_test,200);assert.equal(ds.n_features,null);
...
async function verifyDataset(data,label) {
  ...
  if(label.startsWith('SYNTHETIC COMPLETE')) {
    ...
    assert.equal(inherited.n_failures,1);assert.equal(inherited.review_transitions.source_failure_rows,1);
    assert.equal(inherited.review_transitions.review_stage_failure_rows,0);
    Object.assign(h.api.state,{dataset:'trec',shots:'4',model:'gpt-6-astra',view:'pairs',metric:'accuracy',nativeBudget:'4'});
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(h.api.state)),{dataset:'sst2',shots:'4',model:'all',metric:'accuracy',view:'pairs',nativeBudget:'4'});
  const urlState = {dataset:'trec',shots:'0',model:sourceModels[1].id,metric:'macro_f1',view:'direct',nativeBudget:'full'};
...
async function main() {
  ...
  const complete=completeFixture(), partial=partialFixture(complete), sourceReady=partialFixture(complete,true);
  ...
  const otherStatus=structuredClone(partial);otherStatus.costs.status='waiting_for_local_execution';
  const statusHarness=await harness(otherStatus);assert.match(statusHarness.elements.get('status').textContent,/study in progress; unfinished scores are unavailable/);
