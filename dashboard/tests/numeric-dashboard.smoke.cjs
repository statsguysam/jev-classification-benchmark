...
const pageScripts = loadDecisionPageScripts(dist, html, 'numeric.js');
...
async function harness(data, query = '') {
  ...
  const context = vm.createContext({
    ...
    async fetch(url) {
      assert.equal(url, 'numeric-data.json');
      capture.fetches++;
      return {
        ok: true,
        async json() {
...
async function verifyView(h, data, state) {
  ...
  assert.equal(circles.length, available.length);
  assert.equal(diamonds.length, available.length);
...
function partialFixture(complete) {
  ...
  const pending = data.runs.filter(r => r.arm === 'review' && modelIds.slice(2, 4).includes(r.source_model) && !(r.dataset === 'breast_cancer' && r.source_model === modelIds[2] && r.train_per_class === 0));
  ...
  data.completion.complete_runs = 61;
  data.completion.complete_comparisons = data.comparisons.filter(c => c.status === 'complete').length;
  assert.equal(data.completion.complete_comparisons, 54);
  data.costs.status = 'halted';
  return data;
}
function knownMeasuredReferences(data) {
  const sourceCounts = {
    breast_cancer: [[43, 70], [70, 98], [43, 43], [43, 70], [86, 104], [113, 112]],
    wine: [[12, 12], [14, 29], [12, 12], [10, 12], [17, 32], [36, 35]]
  };
  const nativeCounts = {
    breast_cancer: [[108, 113], [109, 112], [101, 111], [105, 110]],
    wine: [[35, 35], [34, 36], [26, 34], [30, 34]]
  };
