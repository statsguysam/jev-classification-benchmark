...
// --synthetic-only exercises complete and 61/68 partial UI fixtures without a
// numeric-data.json. These invented scores are UI tests, never measurements.
...
const source = fs.readFileSync(path.join(dist, 'numeric.js'), 'utf8');
const syntheticOnly = process.argv.includes('--synthetic-only');
const asset = syntheticOnly ? null : JSON.parse(fs.readFileSync(path.join(dist, 'numeric-data.json'), 'utf8'));
const originalAsset = JSON.stringify(asset);
const metrics = ['accuracy', 'macro_f1'];
const metricNames = {accuracy: 'Accuracy', macro_f1: 'Macro-F1'};
...
function csvRows(value) {
  ...
  for (let i = 0; i < value.length; i++) {
    ...
    } else if (!quoted && c === '\r') { /* CRLF separator */ }
    else field += c;
  }
  ...
  return rows.map(r => { assert.equal(r.length, head.length, 'CSV width'); return Object.fromEntries(head.map((h,i) => [h,r[i]])); });
}

class Element {
  constructor(id) { this.id = id; this.value = ''; this.textContent = ''; this.options = []; this.listeners = new Map(); this.disabled = false; this._html = ''; }
  set innerHTML(value) {
    this._html = value;
...
async function harness(data, query = '') {
  ...
  const capture = {blob: null, anchor: null, fetches: 0, history: ''};
  const context = vm.createContext({
    URLSearchParams, Blob, console,
    location: {search: query},
    history: {replaceState(_a, _b, url) { capture.history = url; }},
    document: {
      getElementById(id) { assert.ok(elements.has(id), `Unknown DOM id ${id}`); return elements.get(id); },
      querySelector(selector) { assert.equal(selector, '.source-dot'); return legend; },
      createElement(tag) { assert.equal(tag, 'a'); const anchor = {click() { capture.anchor = anchor; }}; return anchor; }
    },
    URL: {createObjectURL(blob) { capture.blob = blob; return 'blob:smoke-fixture'; }, revokeObjectURL(url) { assert.equal(url, 'blob:smoke-fixture'); }},
    setTimeout(callback) { callback(); },
    async fetch(url) { assert.equal(url, 'numeric-data.json'); capture.fetches++; return {ok: true, async json() { return data; }}; }
  });
  assert.match(source, /boot\(\);\s*$/, 'Expected a single trailing boot call');
  vm.runInContext(source.replace(/boot\(\);\s*$/, '') + '\n;globalThis.smoke = {boot, render, exportCSV, state, pairs};', context, {timeout: 1000});
  await context.smoke.boot();
  ...
  return {api: context.smoke, elements, legend, capture};
}
...
function expectedPairs(data, state) {
  ...
  return matches.filter(r => r.arm === 'base' && (state.model === 'all' || r.model === state.model)).map(base => {
    ...
    return {base, review, direct, reference, comparison: comparisons[0]};
  });
}
async function exported(h) {
  h.capture.blob = null; h.capture.anchor = null;
  h.api.exportCSV();
...
async function verifyView(h, data, state) {
  Object.assign(h.api.state, state); h.api.render();
  const expected = expectedPairs(data, state), m = state.metric;
  ...
  const available = expected.filter(p => p.reference[m] != null && p.review[m] != null);
  assert.equal(circles.length, available.length); assert.equal(diamonds.length, available.length);
  const csv = await exported(h); assert.equal(csv.length, expected.length);
  assert.equal(h.capture.anchor.download, `jev-numeric-${state.dataset}-${state.shots}-${m}.csv`);
  let marker = 0;
  for (const [i, item] of expected.entries()) {
    const {base, review, reference, comparison: c} = item, t = review.review_transitions;
    assert.equal(c.dataset, state.dataset); assert.equal(c.equal_new_label_budget, true);
    ...
    if (review[m] != null && reference[m] != null) {
      ...
      close(Number(circles[marker][2]), 52 + i*52, 'Chart row');
      close(Number(diamonds[marker][2]), 46 + i*52, 'Diamond row');
      marker++;
    } else assert.ok(chart.includes(`${pct(reference[m])} → ${pct(review[m])} (unavailable)`));
    assert.ok(chart.includes(`${pct(reference[m])} → ${pct(review[m])}`));
    assert.deepEqual(csv[i], Object.fromEntries(Object.entries({
      dataset: state.dataset, metric: m, shots_per_class: state.shots, source_model: base.model, comparison: c.kind,
      review_status: review[m] == null ? 'pending' : 'complete',
      reference_score: reference[m], review_score: review[m], delta_pp: c[`${m}_delta`] == null ? null : 100*c[`${m}_delta`],
      ...
      corrected_from_source: t?.wrong_to_correct, harmed_from_source: t?.correct_to_wrong,
      pipeline_failures: review.n_failures, source_failure_rows: t?.source_failure_rows,
      review_stage_failure_rows: t?.review_stage_failure_rows, test_rows: review.n_test
    }).map(([k,v]) => [k,String(v ?? '')])));
    if (t) assert.equal(review.n_failures, t.source_failure_rows + t.review_stage_failure_rows);
  }
  const ds = data.datasets.find(d => d.id === state.dataset), direct = expected[0].direct;
  ...
  assert.equal(h.elements.get('status').textContent.includes('paused by billing errors'), data.costs.status === 'halted');
...
async function nullChecks(fixture) {
  const baseState = {dataset:'breast_cancer',shots:'4',model:sourceModels[0].id,metric:'accuracy',view:'pairs',nativeBudget:'4'};
  for (const which of ['review','source','direct']) {
    const data = structuredClone(fixture), state = {...baseState,view:which === 'direct'?'direct':'pairs'};
    const item = expectedPairs(data,state)[0];
    (which === 'review' ? item.review : which === 'source' ? item.base : item.direct).accuracy = null;
    item.comparison.accuracy_delta = null; item.comparison.accuracy_delta_ci95 = null;
    const h = await harness(data); Object.assign(h.api.state,state); h.api.render();
    const graphic = h.elements.get('chart').innerHTML;
    assert.ok(graphic.includes('(unavailable)'));
    assert.equal((graphic.match(/<circle /g)||[]).length,0,'Unavailable row must not plot a zero reference');
    assert.equal((graphic.match(/<path /g)||[]).length,0,'Unavailable row must not plot a zero review');
  ...
  const h = await harness(data); Object.assign(h.api.state,baseState); h.api.render();
  let row = (await exported(h))[0];
  assert.equal(row.reference_score,'0'); assert.equal(row.review_score,'0'); assert.equal(row.delta_pp,'0');
  assert.equal(row.delta_ci95_low_pp,''); assert.equal(row.delta_ci95_high_pp,'20');
  ...
  item.comparison.accuracy_delta = 0.1; item.comparison.accuracy_delta_ci95 = [0.2,0.3];
  h.api.render(); row = (await exported(h))[0];
  assert.equal(row.delta_pp,'10'); assert.equal(row.delta_ci95_low_pp,'20'); assert.equal(row.delta_ci95_high_pp,'30');
...
function validateAsset(data) {
  ...
  assert.equal(data.completion.expected_runs,68); assert.equal(data.completion.expected_comparisons,72);
  assert.equal(data.runs.length,68); assert.equal(data.comparisons.length,72);
  ...
  if (data.completion.status === 'complete') {
    assert.equal(data.completion.complete_runs,68); assert.equal(data.completion.complete_comparisons,72);
    assert.equal(data.costs.status,'complete');
  } else {
    assert.ok(data.completion.complete_runs < 68); assert.ok(data.completion.complete_comparisons < 72);
  ...
  for (const r of data.runs) {
    ...
    if (r.arm === 'review') {
      ...
      assert.equal(base.arm,'base'); assert.equal(base.dataset,r.dataset);
      assert.equal(base.train_per_class,r.train_per_class); assert.equal(base.model,r.source_model);
    }
    if (r.status !== 'complete') {
      assert.equal(r.arm,'review','This partial export keeps completed sources and references');
      for (const field of ['accuracy','macro_f1','accuracy_ci95','macro_f1_ci95','review_transitions','n_failures'])
        assert.equal(r[field],null,`Pending ${r.run_id} ${field} must be null`);
    } else for (const m of metrics) {
      assert.equal(typeof r[m],'number'); assert.ok(r[m]>=0 && r[m]<=1);
      assert.equal(r[`${m}_ci95`].length,2);
    }
  }
  for (const c of data.comparisons) {
    const a=byId.get(c.a), b=byId.get(c.b); assert.ok(a && b,'Contrast endpoints must resolve');
    const complete=a.status==='complete' && b.status==='complete';
    assert.equal(c.status,complete?'complete':'pending');
    for (const m of metrics) if (complete) close(c[`${m}_delta`],a[m]-b[m],'All contrast values agree with endpoint scores');
    else { assert.equal(c[`${m}_delta`],null); assert.equal(c[`${m}_delta_ci95`],null); }
...
function completeFixture() {
  const data={schema_version:1,completion:{status:'complete',complete_runs:68,expected_runs:68,complete_comparisons:72,expected_comparisons:72},
    models:modelIds.map((id,i)=>({id,label:id,family:i<4?'open_weight_llm':'hosted_llm'})),
    datasets:[{id:'breast_cancer',label:'Synthetic binary fixture',n_test:114,n_classes:2,n_features:30,full_training_labels:341},
      {id:'wine',label:'Synthetic multiclass fixture',n_test:36,n_classes:3,n_features:13,full_training_labels:106}],
    runs:[],comparisons:[],costs:{status:'complete',new_review_calls:1500,known_reported_api_usd:'1.2',unknown_cost_calls:1,cumulative_reserved_usd:'23.35',authorized_usd:'25'},
  ...
  function add(ds,model,arm,shots,correct) {
    const accuracy=correct/ds.n_test, macro_f1=accuracy-.04;
    const r={run_id:`fixture::${ds.id}::${model}::${arm}::${shots}`,dataset:ds.id,model,display_model:model,arm,status:'complete',
      train_per_class:shots,train_labels:shots===null?ds.full_training_labels:shots*ds.n_classes,
      label_budget:shots===null?'full_training':shots===0?'zero_shot':'four_per_class',accuracy,macro_f1,
      accuracy_ci95:[Math.max(0,accuracy-.05),Math.min(1,accuracy+.05)],macro_f1_ci95:[Math.max(0,macro_f1-.05),Math.min(1,macro_f1+.05)],
      n_test:ds.n_test,n_failures:0,review_transitions:null}; data.runs.push(r); return r;
  }
  function contrast(kind,a,b,equal) {
    const c={kind,dataset:a.dataset,a:a.run_id,b:b.run_id,status:'complete',equal_new_label_budget:equal,
      train_per_class_a:a.train_per_class,train_per_class_b:b.train_per_class,
      transitions:kind==='review_minus_source'?a.review_transitions:null};
    for(const m of metrics){c[`${m}_delta`]=a[m]-b[m];c[`${m}_delta_ci95`]=[Math.max(-1,c[`${m}_delta`]-.1),Math.min(1,c[`${m}_delta`]+.1)];}
  ...
  for (const ds of data.datasets) {
    for (const shots of [0,4]) {
      ...
      for(const [i,model] of modelIds.entries()) {
        ...
        const base=add(ds,model,'base',shots,correct), delta=[-3,-1,0,1,2,3][i];
        const review=add(ds,model+'+jev_review','review',shots,correct+delta);
        review.source_model=model;review.source_run_id=base.run_id;
        const fixed=Math.max(0,delta),harmed=Math.max(0,-delta);
        const inherited=ds.id==='wine'&&model==='gpt-6-astra'&&shots===4?1:0;
        base.n_failures=inherited;review.n_failures=inherited;
        review.review_transitions={wrong_to_correct:fixed,correct_to_wrong:harmed,correct_to_wrong_label:harmed,correct_to_failure:0,
          both_correct:correct-harmed,both_wrong:ds.n_test-correct-fixed,changed_predictions:fixed+harmed,
          source_failure_rows:inherited,review_stage_failure_rows:0,net_correct_change:delta,accuracy_delta_pp:100*delta/ds.n_test};
        contrast('review_minus_source',review,base,true);contrast('review_minus_jev_alone',review,direct,true);
      }
    }
    ...
    for(const shots of [4,null])for(const [i,model] of ['logistic_regression','random_forest','xgboost','lightgbm'].entries())
      add(ds,model,'classical',shots,Math.floor(ds.n_test*.8)+i);
  }
  return data;
}

function partialFixture(complete) {
  const data=structuredClone(complete);
  const pending=data.runs.filter(r=>r.arm==='review'&&modelIds.slice(2,4).includes(r.source_model)
    &&!(r.dataset==='breast_cancer'&&r.source_model===modelIds[2]&&r.train_per_class===0));
  assert.equal(pending.length,7);
  for(const [i,r] of pending.entries()) {
    r.status=i===0?'stopped_after_three_consecutive_errors':'pending: no complete audited artifact';
  ...
  for(const c of data.comparisons)if(pendingIds.has(c.a)||pendingIds.has(c.b)) {
    c.status='pending';c.transitions=null;
    for(const m of metrics){c[`${m}_delta`]=null;c[`${m}_delta_ci95`]=null;}
  }
  data.completion.status='in_progress_or_incomplete';data.completion.complete_runs=61;
  data.completion.complete_comparisons=data.comparisons.filter(c=>c.status==='complete').length;
  assert.equal(data.completion.complete_comparisons,54);data.costs.status='halted';
  return data;
}

function knownMeasuredReferences(data) {
  const sourceCounts={breast_cancer:[[43,70],[70,98],[43,43],[43,70],[86,104],[113,112]],
    wine:[[12,12],[14,29],[12,12],[10,12],[17,32],[36,35]]};
  const nativeCounts={breast_cancer:[[108,113],[109,112],[101,111],[105,110]],wine:[[35,35],[34,36],[26,34],[30,34]]};
  const nativeModels=['logistic_regression','random_forest','xgboost','lightgbm'];
  for(const ds of data.datasets) {
    assert.equal(ds.n_test,ds.id==='wine'?36:114);
    for(const [i,model] of modelIds.entries())for(const [j,shots] of [0,4].entries()) {
      const r=data.runs.find(r=>r.dataset===ds.id&&r.arm==='base'&&r.model===model&&r.train_per_class===shots);
      close(r.accuracy,sourceCounts[ds.id][i][j]/ds.n_test,'Frozen measured source accuracy');
    }
    for(const [i,model] of nativeModels.entries())for(const [j,shots] of [4,null].entries()) {
      ...
      close(r.accuracy,nativeCounts[ds.id][i][j]/ds.n_test,'Frozen measured native accuracy');
    }
    for(const [j,shots] of [0,4].entries()) {
      const r=data.runs.find(r=>r.dataset===ds.id&&r.arm==='direct'&&r.train_per_class===shots);
      close(r.accuracy,(ds.id==='wine'?[12,33]:[96,106])[j]/ds.n_test,'Frozen measured direct Jev accuracy');
    }
  }
}
...
async function verifyDataset(data,label) {
  validateAsset(data);
  const original=JSON.stringify(data), h=await harness(data);
  ...
  let views = 0;
  for (const dataset of data.datasets.map(d => d.id)) for (const shots of ['0','4'])
    for (const model of ['all',...sourceModels.map(m => m.id)]) for (const view of ['pairs','direct'])
      for (const metric of metrics) for (const nativeBudget of ['4','full']) {
        const state = {dataset,shots,model,view,metric,nativeBudget};
        try { await verifyView(h,data,state); } catch (e) { e.message = `${label} ${JSON.stringify(state)}: ${e.message}`; throw e; }
        views++;
      }
  assert.equal(views,224);
  // Regression: this inherited source failure incurred no Jev request.
  const inherited = data.runs.find(r => r.dataset === 'wine' && r.arm === 'review' && r.source_model === 'gpt-6-astra' && r.train_per_class === 4);
  assert.ok(inherited); assert.equal(inherited.n_failures,1);
  ...
  Object.assign(h.api.state,{dataset:'wine',shots:'4',model:'gpt-6-astra',view:'pairs',metric:'accuracy',nativeBudget:'4'});
  h.api.render(); const inheritedCSV = (await exported(h))[0];
  assert.equal(inheritedCSV.pipeline_failures,'1'); assert.equal(inheritedCSV.source_failure_rows,'1'); assert.equal(inheritedCSV.review_stage_failure_rows,'0');
  // Exercise actual event handlers, reset, and URL restoration, not only render().
  h.elements.get('metric').value = 'macro_f1'; h.elements.get('metric').listeners.get('change')();
  assert.equal(h.api.state.metric,'macro_f1');
  h.elements.get('reset').listeners.get('click')();
  assert.deepEqual(JSON.parse(JSON.stringify(h.api.state)),{dataset:'breast_cancer',shots:'4',model:'all',metric:'accuracy',view:'pairs',nativeBudget:'4'});
  const urlState = {dataset:'wine',shots:'0',model:sourceModels[1].id,metric:'macro_f1',view:'direct',nativeBudget:'full'};
...
async function main() {
  if(asset){knownMeasuredReferences(asset);await verifyDataset(asset,'REAL EXPORTED ASSET');}
  const complete=completeFixture(), partial=partialFixture(complete);
  await verifyDataset(complete,'SYNTHETIC COMPLETE UI FIXTURE (not measurements)');
  await verifyDataset(partial,'SYNTHETIC PARTIAL UI FIXTURE (not measurements)');
