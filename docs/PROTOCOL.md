...
See [the verified inventory](SOURCES.md#model-selection-audit). Pin Jev to **`jev-1.13.0`**. Main practical open model: `Qwen/Qwen3-4B-Instruct-2507`; the smaller Qwen configuration is a technical smoke baseline. Fix open-model/tokenizer revision, precision, adapter, and scoring method. Resolve missing/moving revisions before publication and do not mix resolved versions when resuming.
