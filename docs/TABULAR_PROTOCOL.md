...
Local model work and hosted dataset workers overlapped during execution. Client load, network conditions, provider serving and guard overhead were not held constant across timing measurements. The recorded latency values describe these executions; they are not a controlled throughput comparison or an architectural speed ranking.
