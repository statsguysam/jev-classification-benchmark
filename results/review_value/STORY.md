...
A large gain after review does not tell us whether both models were needed. This pilot asks two questions: **does Jev improve the source model's answers, and does the source model help Jev?** The reports compare both stages alone with the pipeline and count corrections, harmful overrides and failed responses.
...
## The large Wine gain also appeared with Jev alone
...
But **all 36 reviewed labels matched the labels from Jev alone**, which also scored 33/36. The source had predicted one class throughout the test fold. Jev corrected much of that output, but keeping the source in the pipeline added no observed accuracy over direct Jev in this condition. The historical direct and review prompts also differ, so this comparison alone cannot isolate the proposal's effect. See the [complete numerical findings](FINDINGS.md) and [numeric evidence audit](../completion_20260923/NUMERIC_EVIDENCE_REVIEW.md).
...
Review reduced the number of accepted correct answers in all eight Astra conditions, covering four datasets at two example settings. These are **four reused test sets, not eight independent replications**, and counting the direction of change is not a significance test. The [Astra review audit](../completion_20260923/ASTRA_REVIEW_AUDIT.md) publishes all eight rows, correction/harm partitions and source-file hashes.
...
## Retrying failed requests did not reverse the Astra result
...
## Simulating review for only some rows
...
## What the scoring method and test data can tell us
...
Adding the real proposal produced observed accuracy differences of **0, 0, +0.5 and −0.5 percentage points versus no proposal**. Every paired interval spans zero, so the experiment did not show a reliable accuracy gain from the proposal. A benefit or harm could still emerge with more data, especially on Wine, where the interval is wide. The actual-versus-shuffled results vary in direction; the TREC interval lies below zero, but these eight exploratory intervals are unadjusted and conditional on one shuffle. They do not establish that shuffling is a better general strategy.
